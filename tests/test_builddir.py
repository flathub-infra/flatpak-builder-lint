...
def test_builddir_appid() -> None:
    ...
    ret = run_checks("tests/builddir/appid", "builddir")
...
def test_builddir_url_not_reachable(mock_domainutils: dict[str, MagicMock]) -> None:
    ...
    ret = run_checks("tests/builddir/wrong-rdns-appid", "builddir")
    found_errors = set(ret["errors"])
...
def test_builddir_finish_args() -> None:
    ...
    ret = run_checks("tests/builddir/finish_args", "builddir")
    found_errors = set(ret["errors"])
...
def test_builddir_finish_args_new_metadata() -> None:
    ret = run_checks("tests/builddir/finish_args_new_metadata", "builddir")
    found_errors = set(ret["errors"])
...
def test_builddir_display_supported() -> None:
    ...
    ret = run_checks("tests/builddir/display-supported", "builddir")
    found_errors = set(ret["errors"])
    for a in absents:
        assert a not in found_errors
...
def test_builddir_finish_args_missing() -> None:
    ret = run_checks("tests/builddir/finish_args_missing", "builddir")
    found_errors = set(ret["errors"])
...
def test_builddir_flathub_json() -> None:
    ...
    move_files(testdir)
    ret = run_checks(testdir, "builddir")
    found_errors = set(ret["errors"])
...
def test_builddir_metadata_spaces() -> None:
    run_checks("tests/builddir/metadata-spaces", "builddir")
...
def test_min_success_metadata() -> None:
    ...
    ret = run_checks(cli_testdir, "builddir")
    found_errors = set(ret["errors"])
...
def test_builddir_dconf_access() -> None:
    ret = run_checks("tests/builddir/dconf-access", "builddir")
    found_errors = set(ret["errors"])
    errors = {
...
def test_builddir_xdg_dir_access() -> None:
    ret = run_checks("tests/builddir/finish_args_xdg_dirs", "builddir")
    found_errors = set(ret["errors"])
    errors = {
...
def test_builddir_home_host_access() -> None:
    ...
    for i in range(1, 6):
        ret = run_checks(f"{base_path}/home_host{i}", "builddir")
        found_errors = set(ret["errors"])
        assert "finish-args-home-filesystem-access" in found_errors

    ret = run_checks(f"{base_path}/home_host1", "builddir")
    found_errors = set(ret["errors"])
    assert "finish-args-host-filesystem-access" in found_errors

    ret = run_checks(f"{base_path}/home_host6", "builddir")
    found_errors = set(ret["errors"])
    assert "finish-args-home-ro-filesystem-access" in found_errors

    ret = run_checks(f"{base_path}/home_host7", "builddir")
    found_errors = set(ret["errors"])
    assert "finish-args-host-ro-filesystem-access" in found_errors

    ret = run_checks(f"{base_path}/home_host_false", "builddir")
    found_errors = set(ret["errors"])
