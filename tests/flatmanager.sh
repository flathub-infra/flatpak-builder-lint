...
run_test() {
    ...
    local result
    result="$(flatpak run --env=FLATPAK_BUILDER_LINT=skip-eol-runtime-checks,skip-policy-enforcement --command=flatpak-builder-lint org.flatpak.Builder//localtest --exceptions repo repo \
        | jq -r '.errors|.[]' | xargs)"
